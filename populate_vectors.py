...
conn = oracledb.connect(user=os.environ["DB_USER"], password=os.environ["DB_PASSWORD"], dsn=CONNECTION_STRING)
cursor = conn.cursor()
...
if not rows:
    print("No work to be done! Either table is empty, or all records already have vector embeddings populated.")
    cursor.close()
    conn.close()
    exit()
